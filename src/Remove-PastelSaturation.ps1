@{
    ...
    Synopsis                = "Decrease color saturation by a specified amount"
    Description             = "Decrease the saturation of a color by subtracting a certain amount from the HSL saturation channel. If the amount is negative, the color will be saturated instead."
    Parameters              = @{
        ParameterType                   = "decimal"
        Name                            = "Amount"
        Description                     = "Amount of saturation to subtract (number between 0.0 and 1.0)"
    ...
    Examples                = @{
        Command         = "Remove-PastelSaturation -Amount 0.1 -Color red"
        OriginalCommand = "pastel desaturate 0.1 red"
